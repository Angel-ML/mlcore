...
package com.tencent.angel.mlcore.optimizer.loss
...
import com.tencent.angel.mlcore.utils.LossFuncKeys
...
object LossFunc {
  private[mlcore] def fromJson(jast: JValue): LossFunc = {
    jast match {
...
class KmeansLoss extends LossFunc {
  ...
  override def predict(modelOut: Matrix, graph: Graph): List[PredictResult] = {
    ...
    val data = graph.placeHolder.getRawData
    modelOut match {
      case m: BlasDoubleMatrix =>
        (0 until m.getNumRows).foreach { idx =>
          ...
          result.append(mlcore.PredictResult(sid, pred, proba, predLabel, trueLabel, attached))
        }
      case m: BlasFloatMatrix =>
        (0 until m.getNumRows).foreach { idx =>
          val labeledData = data(idx)
